...
import ru.netology.coroutines.dto.CommentTotal
import ru.netology.coroutines.dto.PostTotals
    ...
    withContext(Dispatchers.IO) {
        client.apiCall(url)
            .let { response ->
                ...
                // Заменил т.к. неправильно выводил на печать (видимо по у молчанию система ожидает что сервер будет грузить в UTF-8)
                val bytes = response.body.bytes()
                val decodedString = String(bytes, Charset.forName("windows-1251"))

                gson.fromJson(decodedString, typeToken.type)
            }
    }
...
suspend fun getComments(client: OkHttpClient, id: Long): List<CommentTotal> =
    makeRequest("$BASE_URL/api/slow/posts/$id/comments", client, object : TypeToken<List<Comment>>() {})
        .map { comment ->
            CommentTotal(comment, getAuthors(client, comment.authorId))
        }
